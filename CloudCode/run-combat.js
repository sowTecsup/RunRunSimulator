...
module.exports = async ({ params, context, logger }) => {
    ...
    let pool = [];
    try {
        const res = await api.getCustomItem(context.projectId, context.environmentId, POOL_KEY);
        pool = JSON.parse(res.data?.value ?? "[]");
    } catch (_) {
        // Pool doesn't exist yet — start empty
    }
    ...
    if (opponentIdx === -1) {
        ...
        await api.setCustomItem(context.projectId, context.environmentId, POOL_KEY, {
    ...
    await appendResult(api, context.projectId, context.playerId,  buildResult(battle, true,  opponent.customName));
    await appendResult(api, context.projectId, opponent.playerId, buildResult(battle, false, customName));
...
function buildResult(battle, callerIsA, opponentName) {
