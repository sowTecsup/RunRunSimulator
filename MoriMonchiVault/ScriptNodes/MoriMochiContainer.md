...
Corral base con `BoxCollider` trigger. Implementa `IAnchorPlace` para generalizar "lugar donde una criatura está anclada". En `Start()` deriva el `AnchorKey` del `PlacedFurnitureMarker` y se auto-registra en `AnchorRegistry`. En `OnDestroy()` desregistra.
...
Expone `Occupants` (IReadOnlyList) y tabla `OccupantInfos` para inspector (nombre/género/personalidad). `Claim()` protegido es compartido por admisión y `BreedingContainer`. `EnterConfinement()` confina al agente (cambia areaMask).
