...
**Responsabilidad:** Apex del sistema de combate (domain owner). Singleton: `static Instance`. Posee `config` (CombatManagerSO serializado). Public API: getter `Config` (lectura del SO), `SimulateLocal()` (combate local + evento, pasa `GameManager.Instance?.EquipmentDatabase` a CombatService.Simulate para resolver procs), `EnqueueForAsyncCombat()` + wrappers async (DequeueAsync, PollResultsAsync, FetchQueuedIdsAsync, FetchPendingResultIdsAsync). Dev tooling vive en `CombatDevConsole`. Resuelve registry y database de GameManager.Instance en Awake.
