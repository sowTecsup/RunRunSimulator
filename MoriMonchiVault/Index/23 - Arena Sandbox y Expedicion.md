...
- Etapa 3 (`Index/22` 8.9): paso 2 = UI de decisión de acciones (exige el catálogo `ExpeditionObjective/Role/DirectiveSO` y las 2 respuestas ⭐ de 8.7); después paso 4 nervio y memoria corta, paso 5 utilidad con curvas, paso 6 cono de visión.
...
- Percepción 9 m desde S101 (antes 6): los minerales de esquina siguen fuera del radio para quien está en el centro; decisión de diseño pendiente (8.7). El knob es global (`SocialTuning.asset`): si molesta en la tienda, hace falta un `SocialTuning_Arena` (y cuidar el singleton `Current`).
