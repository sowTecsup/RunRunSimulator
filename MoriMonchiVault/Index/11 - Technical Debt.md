...
- **(S95) Deuda de diseño — `Tier`/`Rarity`/HP-Attack-Speed de `BodyPart` y `BodyTier/HornTier/BackTier/WingTier` de `CreatureDNA` contradicen la decisión de Juan** "ninguna parte es intrínsecamente mejor; lo que se hereda es el potencial" ([[Index/21 - Combate v3 - Dragon RPS]] §9.10). Los tiers del DNA son un campo muerto (nadie los asigna ni hereda; las 19 criaturas del JSON los tienen en 0, fuera del enum) pero los leen `ValuationHandler` (precio), `CreatureStats` (stats) y `DetailInfoTabPresenter` (ficha). Decidir: ¿precio y stats pasan a leer potencial, o desaparecen con los perks (Parte 5)? Hasta entonces no borrar.
