...
- [ ] **(S95) ¿Robás 1 tras cada carta o jugás por tandas de 3?** Juan, al ver el prototipo: *"si bien tienes 6 cartas 2/2/2, recibes 3 al azar en un shuffle, en eso habíamos quedado"*. El motor hace hoy la regla de la Parte 3 (mano de 3 barajada, jugás 1 y robás 1; verificado con 8 semillas y 20k manos: 40% traen una de cada tipo, como la teoría — las capturas cayeron justo en ese 40% y por eso parecía que siempre había Cuernos/Alas/Espalda). Variante **B** propuesta para confirmar: recibís 3, las jugás en 3 rondas sin robar, recibís las otras 3, recién ahí se rebarajan las 6. Si Juan elige B: cambio en `DragonRpsMatch.ResolveRound`/`DragonRpsSide` (sin `Draw` tras jugar; `Reshuffle` cuando el deck y la mano se vacían) + volver a medir con el harness (§9.7).
- [ ] **(S95) Las criaturas deberían tener su propia escena** (Juan). Propuesta del orquestador, pendiente de OK: reemplazar el `RingStage` dentro del prefab (§9.5) por una **escena aditiva `CombatArena.unity`** que se carga al aceptar la pelea: escenario nocturno, dos spots enfrentados, cámara Cinemachine propia, luz de foco; los dragones se arman con `MonchiVisualizer` + `DragonAnimationDriver`, el panel se reduce a una tira inferior (mano, pips, log, triángulo) y golpes / partes rotas / KO ocurren en la escena; al cerrar se descarga y se vuelve al Ring, que queda como puerta de entrada. Animaciones propuestas en orden: revelado del choque (UITK) → dragones en el escenario → partes que se rompen (`SetPartWorn` + Feel + DamageNumbersPro) → KO/victoria + cámara → rearme de la mano.
