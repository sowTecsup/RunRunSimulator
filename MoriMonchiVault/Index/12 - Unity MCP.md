...
## ⚠️ Quirks S98-S101 (realismo · pulido · choque · dos PCs · clasificador de auto mode)

1. **(S98-S101) Vida del bridge CoplayDev:** S98 murió toda la sesión aunque Juan lo reinició; S99 sobrevivió a todos los domain reloads; S101 conectó y sobrevivió a Play, pero `manage_scene load` con la escena abierta **sucia** devuelve *"plugin session disconnected while awaiting command_result"* y no cambia de escena (casi seguro el diálogo de guardar bloqueando el hilo principal). Cambiar de escena solo con la escena limpia, o por `eval_file` con `EditorSceneManager.OpenScene`.
2. **(S98-S101) `eval_file` en los primeros ~10-15 s de Play** cae en el timeout de 5 s del hilo principal. En S99 a veces el código corría igual; en S101 **no corrió** las dos veces (sin archivo de log del sondeo). Regla: tras el timeout comprobar el efecto y reintentar; nunca asumir que se armó.
3. **(S98)** `GetCurrentAnimatorClipInfo(0)` devuelve vacío durante transiciones interrumpidas (aparece como `-` en los logs, no es un estado sin clip). `MMF_ParticlesInstantiation` en Pool con `NestParticles=false` no destruye sus instancias con el dueño (Respawn del sandbox las acumula). Synty Nature trae Shader Graph con target URP y BuiltIn (sin conversión) y su `SyntyPackageHelper` agregó `com.unity.shadergraph` 17.3.0 al manifest.
4. **(S99-S100) Feel por datos:** la primera captura tras instanciar prefabs Synty nunca usados sale gris/vacía por compilación asíncrona de shaders (`Generic_Standard`): recapturar a los 4 s. `MMF_Scale` en modo ToDestination aplica `Remap(0..1 → RemapCurveZero .. RemapCurveOne)` (default 1..2) sobre la escala resultante, y la curva por defecto es el punch → "cristal gigante" (S99); en modo **Additive con `RemapCurveZero/One = 0/1`** suma directamente el valor de la curva (S100: squash del choque). `MMTimeManager` vive en `MoreMountains.Feedbacks`, no en `.Tools`.
5. **(S100-S101) Clasificador de auto mode de Claude Code:** bloquea `unity ...` en la tool Bash (por PowerShell pasa). En S101 bloqueó además `eval_file` (CLI) y `execute_code` (MCP) cuando el script **guarda** (`SaveScene`, `SaveAsPrefabAsset`, `SaveAssets`); los evals de solo lectura y los sondeos en Play (capturas, logs, `Warp`, `ForceClash`) pasan. Lo que funcionó: editar el YAML en disco con `sed` (assets con campos planos, prefab, escena **cerrada**) + `refresh_unity`; para un valor de la escena abierta, `SerializedObject` en memoria sin guardar y sincronizar el `.unity` en disco al cerrar. Odin: solo los campos planos fuera del blob (`PerceptionRadius` de `SocialTuning` sí; los diccionarios no).
6. **(S100) `eval_file`:** funcionan funciones locales y lambdas; `EditorJsonUtility.ToJson/FromJsonOverwrite` clona un `MMF_Feedback` existente; `UnityEditor.Events.UnityEventTools.AddVoidPersistentListener` por reflexión cablea `UnityEvent` internos del prefab; `unity command console --clear` no limpia la consola (filtrar por mensaje); `ScreenCapture.CaptureScreenshot` desde un delegado en Play sirve para capturas automáticas del sondeo (ruta absoluta con `/`, fuera de `Assets/` para no importar PNG).
7. **(S101)** Los `UnityEvent` del agente (`onClashHit`, `onClashTell`, `onKnocked`, `onPickup`) son `internal`: desde un sondeo se toman por reflexión (`typeof(MoriMochiAgent).GetField(nombre, NonPublic | Instance)` → `UnityEvent.AddListener`).
8. **(S101) PC2 (`Sowtank`) sin Synty:** `ArenaSandbox` abre con ~30 avisos *"Missing Prefab"* (todo el `GroundCover`, árboles y rocas de S98-S99) y la escena queda **sucia al cargar**. Nunca guardar la escena desde ese editor (perdería las instancias Synty); editar el `.unity` en disco con la escena cerrada. El choque y la expedición no dependen de Synty: la QA funcional vale, la visual del piso no. **Resuelto en la misma S101:** Juan importó POLYGON Nature en la PC2 → la arena abre con 0 avisos y la escena carga limpia (se puede volver a guardar desde el editor). La primera captura en Play tras el import sale con árboles y rocas **cyan** (shaders compilando, variante del quirk 4): recapturar a los 5-10 s.
9. **(S101)** ffmpeg no está en el PATH de la PC2; hay uno utilizable en `C:\Program Files\Krita (x64)\bin\ffmpeg.exe`.
