...
**Decisiones S107 (Juan, sesión remota):** (1) el señuelo **queda como postura de apoyo** a Muralla/Emboscada; no se le da sitio dinámico (Hormiguero+Señuelo 52 % y Señuelos 24 % quedan como pisos por construcción). (2) Feedback sobre el video "del plan al combate": la pantalla ya dice lo que hace falta para leer sala y rival, pero el jugador arma solo el puente "por eso"; se pidió **descubrimiento** (rivales revelados solo al pelear o recolectar; provocar no revela), azul pastel aliados / rojo pastel rivales, menos texto en pantalla (la base de color basta para diferenciar), cono punteado y más transparente, rival a la derecha con retrato girando y su naturaleza, y tres poderes automáticos con radial de carga. Todo implementado en S107 (`Index/23` 5k).

### 8.12 · Habilidades automáticas por parte, al estilo Pokémon Quest (S104 · Juan ⭐ · v1 implementada en S107: base de datos + `AgentAbilities` + HUD radial, ver `Index/23` 5k)
