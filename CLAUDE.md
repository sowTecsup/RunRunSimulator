...
| Unity MCP + Unity CLI (editor en vivo) | Leer/editar escena, wirear SOs, correr C#, Play mode, consola, ProBuilder (MCP) · compilar/consola/capturas con HUD/eval/tests/builds (CLI `unity`). **Verificar en el editor antes de declarar hecho.** Matriz de decision, how-to y quirks en [[MoriMonchiVault/Index/12 - Unity MCP]]. |
...
├── Index/                     ← notas por dominio y diseno (01-20 · 12 = Unity MCP+CLI · 20 = MVP combate · 09b = digest historico)
└── ScriptNodes/               ← ~200 nodos, uno por script .cs
```
...
11. **Composicion sobre partial (decision S32)**: un script grande se divide en **partes pequenas que componen el todo** — mini-managers/colaboradores con estado propio y UNA responsabilidad, coordinados por un nucleo delgado. Patron canonico vivo: World/AI post-S55 (`AgentBrain`/`AgentPhysics`/`AgentConfinement`/`AgentSocial`/`AgentSenses` componen `MoriMochiAgent`; `ControllerPool`/`SpawnBallistics`/`SpawnDiagnostics` componen `MoriMochiSpawner`). `partial` NO es remedio al tamano: sigue siendo UNA clase con un solo estado mutable, esconde lineas sin reducir acoplamiento. Uso legitimo de `partial`: SOLO ventaja fisica de archivo (conflictos de Git, codigo autogenerado). Codigo puro (matematica, helpers sin estado) va en clase estatica aparte; tooling dev que usa API publica va en componente aparte (caso F3: DevConsoles con refs serializadas). **La deuda de partials esta PAGADA** (Fases 6-9 de [[MoriMonchiVault/Index/11 - Technical Debt]], cerradas S53-S55; verificado S90: cero `partial class` en el codigo) — no reintroducir partials nuevos.
...
- Se dispara **solo via `/cerrar-sesion`** (Juan lo invoca); nunca automatico al cierre por iniciativa de la IA.
