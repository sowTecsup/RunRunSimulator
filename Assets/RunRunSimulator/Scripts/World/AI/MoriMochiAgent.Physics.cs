...
public partial class MoriMochiAgent
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (other != null && !ReferenceEquals(other, this) && !IsBreeding)
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (hit == null || ReferenceEquals(hit, this) || IsBreeding) return;
    ...
    public void Knock(Vector3 force)
    {
        ...
        if (currentContainer != null || IsBreeding) return;   // penned or incubating: tackle-proof — only the player can take it out
    ...
    private void RecoverIfStuckOffMesh()
    {
        bool stuck = IsNavMeshControlled() && rb.isKinematic && (!agent.enabled || !agent.isOnNavMesh);
        if (!stuck) { offMeshGrace = 0f; return; }

        offMeshGrace += Time.deltaTime;
        if (offMeshGrace < offMeshRecoverDelay) return;
        offMeshGrace = 0f;

        if (currentContainer != null)
        {
            if (RejoinNavMesh(currentContainer.Center, confinedAreaMask)) EnterRoaming();
